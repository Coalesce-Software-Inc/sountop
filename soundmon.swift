...
func displayProcesses(processes: [AudioProcess], showAll: Bool, logMode: Bool) {
    ...
    if !logMode {
        ...
        let title = " soundmon - Audio Process Monitor "
...
func printUsage() {
    print("""
    soundmon - Monitor audio output by process on macOS
    ...
    EXAMPLES:
        soundmon                   # Monitor with 1 second interval
        soundmon -i 0.5            # Monitor every 500ms
        soundmon -l -i 2           # Log mode, every 2 seconds
        soundmon -1                # Single snapshot
        soundmon -a -n             # Show all, no colors
    """)
}
